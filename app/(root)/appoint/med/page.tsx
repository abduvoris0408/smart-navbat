...
const OrganizationsList: React.FC = () => {
	...
	const [filteredMedicalOrgs, setFilteredMedicalOrgs] = useState<
		Organization[]
	>([])
	...
	// Medical-related terms to filter by
	const medicalTerms = [
		'klinika',
		'shifoxona',
		'poliklinika',
	...
	// Function to check if an organization is medical-related
	const isMedicalOrganization = (org: Organization): boolean => {
		// Check name for medical terms
		const nameMatches = medicalTerms.some(term =>
			org.name.toLowerCase().includes(term.toLowerCase())
		)

		// Check services for medical terms
	...
	useEffect(() => {
		const fetchOrganizations = async () => {
			...
			try {
				...
				// Filter only medical organizations
				const medicalOrgs = data.filter(isMedicalOrganization)
				setFilteredMedicalOrgs(medicalOrgs)
	...
	// Mavjud xizmatlar turlarini olish (only from medical organizations)
	const allServices = filteredMedicalOrgs.flatMap(org => org.services)
	const uniqueServices = [...new Set(allServices)]

	// Tanlangan filter bo'yicha tibbiy tashkilotlarni filtrlash
	const displayedOrganizations =
		selectedFilter === 'all'
			? filteredMedicalOrgs
			: filteredMedicalOrgs.filter(org =>
	...
	return (
		<main className=' mx-auto py-[90px]'>
			...
			{loading ? (
				...
				<div className='text-center py-10'>
					<p className='text-muted-foreground text-lg'>
						Bu filter bo`yicha tibbiy tashkilotlar topilmadi
					</p>
				</div>
			)}
